...
const strengthLevels = {
  0: { text: '', color: 'bg-slate-200 dark:bg-slate-700', width: '0%' },
  ...
  return (
    <div className="space-y-2 transition-all duration-300">
      <div className="flex items-center justify-between">
        <p className="text-sm font-medium text-slate-700 dark:text-slate-300">Password Strength</p>
        <p className={`text-sm font-bold`} style={{ color: score > 0 ? level.color.replace('bg-', '') : '' }}>
          {level.text}
        </p>
      </div>
      ...
      <ul className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1 pt-1 text-sm text-slate-600 dark:text-slate-400">
