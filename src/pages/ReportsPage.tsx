  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      <div className="animate-fade-in-up relative z-30">
        <Card>
          <label htmlFor="date-filter" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Select Period</label>
          <PremierDatePicker
            value={dateFilter}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="animate-fade-in-up" style={{ animationDelay: '100ms' }}><Card><h4 className="text-slate-500 dark:text-slate-400">Total Income</h4><p className="text-2xl font-bold text-emerald-600">Rs. {summary.income.toLocaleString('en-IN')}</p></Card></div>
        <div className="animate-fade-in-up" style={{ animationDelay: '200ms' }}><Card><h4 className="text-slate-500 dark:text-slate-400">Total Expense</h4><p className="text-2xl font-bold text-rose-600">Rs. {summary.expense.toLocaleString('en-IN')}</p></Card></div>
        <div className="animate-fade-in-up" style={{ animationDelay: '300ms' }}><Card><h4 className="text-slate-500 dark:text-slate-400">Top Category</h4><p className="text-2xl font-bold text-slate-900 dark:text-slate-100 truncate">{summary.topCategory}</p></Card></div>
      </div>

      <div className="animate-fade-in-up" style={{ animationDelay: '400ms' }}>
        {showTrendChart ? (
          <Card>
            <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100 mb-4">Income vs Expense Trend</h3>
            <div className="text-slate-600 dark:text-slate-400">
              <ResponsiveContainer width="100%" height={300}>
                <LineChart data={chartData}>
                  ...
                  <Tooltip contentStyle={{ backgroundColor: 'rgba(255, 255,255, 0.8)' }} wrapperClassName="dark:!bg-slate-800 dark:!border-slate-700" />
                  <Legend />
          ...
          <Card>
              <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100 mb-4">Daily Activity</h3>
      ...
      <div className="animate-fade-in-up" style={{ animationDelay: '500ms' }}>
        <Card>
          <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100 mb-4">Category-wise Spending</h3>
          {pieData.length > 0 ? (
            <ResponsiveContainer width="100%" height={300}>
                <PieChart>
                    ...
                    <Tooltip wrapperClassName="dark:!bg-slate-800 dark:!border-slate-700" />
                    <Legend />
                </PieChart>
            </ResponsiveContainer>
          ) : (
